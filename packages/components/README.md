...
- 操作可配置 `permission: 'system:users:edit'`，表格传入 `:has-permission="authorization.hasPermission"` 后按完整权限标识过滤。配置权限但未传判断函数时隐藏该操作；未配置权限的操作保持原有行为。点击时再次校验权限、隐藏和禁用状态。自定义 `#actions` 插槽的权限由页面自行判断。
