...
授权树中主动勾选目录会同步勾选其下所有页面与按钮，主动勾选页面会同步勾选其下所有按钮；取消勾选同样作用于完整子树，包括折叠或被搜索隐藏的后代。保存后统一生效，不自动授予以后新增的权限。
单独勾选按钮只补齐所属页面和祖先目录，不勾选兄弟权限；可逐项取消按钮以保留页面访问权限。回显已有授权不触发整组授予，未操作时保存不会扩大原有权限。列表数据仍需相应“查看”操作权限。
...
错误响应为相同结构，HTTP 状态为 4xx/5xx，code 为六位业务码。例如权限不足为 HTTP 403 / 140001；定义统一维护于 `@common-crm/errors`。
