...
1. 创建目录或页面，手动填写唯一权限标识；页面选择已注册组件并配置其所属子应用内的路径。
...
`componentKey` 只用于选择页面组件；新增页面需维护 `packages/types/src/page-catalog.ts` 与子应用组件注册表。
配置菜单不会凭空生成业务页面或接口。
...
菜单管理页面及其上级目录不能停用或隐藏，菜单管理组件不可替换或删除。
